...
class Card(Base):
    __tablename__ = (
        "cards"  # テーブル名をcardsと指定 __tablename__はsqlalchemyの特別な変数
    )
    __table_args__ = {"extend_existing": True}  # 既存テーブルの再定義を認める
    card_id = Column(String(50), primary_key=True)
    card_pos = Column(Integer(unsigned=True), nullable=False)
    col_id = Column(
        String(50), ForeignKey(Category.col_id, ondelete="CASCADE"), nullable=False
    )
