...
# カテゴリテーブルの要素のうち以下をフロントから受け取る（addの場合と同じもの）
class Request(BaseModel):
    col_id: str = Field(..., title="カテゴリid", description="カテゴリのID(ハッシュ値)")
    col_pos: int = Field(
        ..., title="カテゴリの位置", description="カテゴリの位置を示す番号"
    )
    col_name: str = Field(
        ...,
        title="カテゴリ名",
        description="カテゴリの名前(未着手／完了などのステータス名)",
    )
    description: str = Field(None, title="カテゴリの説明", description="カテゴリの説明")


RequestExample = {
    "col_id": "category-7414208f-560c-4641-8d08-f7e",
    "col_pos": 1,
    "col_name": "TODOリスト",
...
class Response(BaseModel):
    ...
    data: Optional[Data] = Field(None, title="更新したカテゴリの情報")
